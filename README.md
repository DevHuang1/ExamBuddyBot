...
- 💬 **Conversation memory** — remembers recent Q&A so follow-up questions have context, while per-chat update processing keeps rapid messages and replies in order. Edited Telegram messages are ignored so a correction cannot accidentally repeat an expensive study action.
...
| `/flashcards export` | Download the learner’s latest validated flashcard set as a text file; in groups, delivered only to the requester’s private chat |
| `/flashcards anki` | Download the latest set as a tab-separated Anki Basic import file in the `ExamBuddy` deck; in groups, delivered only to the requester’s private chat |
...
| `/studyguide export` | Download the learner’s latest validated revision guide as a text file; in groups, delivered only to the requester’s private chat |
...
| `POLL_INTERVAL_MS` | No | Normal Telegram polling cadence in milliseconds (default `1500`) |
| `MAX_POLL_BACKOFF_MS` | No | Maximum delay after repeated transient Telegram polling failures (default `30000`) |
...
| `FLASHCARD_SETS_FILE` | No | Optional path for the latest private flashcard export per learner workspace (defaults to `/data/flashcard-sets.json` where available) |
| `STUDY_GUIDES_FILE` | No | Optional path for the latest private revision guide per learner workspace (defaults to `/data/study-guides.json` where available) |
